...
void sunlogf_root(
	...
	if (str_global_log_level != NULL && strncmp(str_global_log_level, "none", 5) != 0 &&
		((str_global_log_level != NULL && strncmp(str_global_log_level, "info", 5) == 0 && int_error_level <= 6) ||
			(str_global_log_level != NULL && strncmp(str_global_log_level, "notice", 7) == 0 && int_error_level <= 5) ||
			(str_global_log_level != NULL && strncmp(str_global_log_level, "warn", 5) == 0 && int_error_level <= 4) ||
			int_error_level <= 3)) {
#ifdef _WIN32
		FILE *fp = NULL;
