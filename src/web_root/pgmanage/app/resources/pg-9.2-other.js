    ...
    GS.requestFromSocket(GS.envSocket, 'INFO', function (data, error, errorData) {
        ...
        if (!error) {
            if (data !== 'TRANSACTION COMPLETED') {
                ...
                contextData.pgmanageVersion = arrCells[1];
...
function menuTools(target) {
    ...
    templateElement.innerHTML = ml(function () {/*
        <gs-page>
            <gs-body>
                <gs-button class="pgmanage-menu-item-button" dialogclose no-focus iconleft icon="line-chart" target="_blank"
...
function menuTab(target) {
    ...
    if (window.process && window.process.type === 'renderer') {
        ...
        templateElement.innerHTML = ml(function () {/*
            <gs-page>
                <gs-body>
                    ...
                    <gs-button onclick="saveCurrentScript()" icon="save" iconleft  class="pgmanage-menu-item-button" title="Save" no-focus>Save File</gs-button>
                    <gs-button id="button-tab-current-save-as" onclick="saveCurrentScript(true)" icon="save" iconleft class="pgmanage-menu-item-button button-save-as" title="Save As..." remove-all no-focus>
        ...
        */}).replace(/\{\{TABPATH\}\}/gi,
                ...
                : path.normalize(app.getPath('home') + '/.pgmanage/sql/').replace(/\\/g, '\\\\')
            );
    } else {
        templateElement.innerHTML = ml(function () {/*
            <gs-page>
    ...
    */});
    ...
    GS.openDialogToElement(target, templateElement, 'down', function () {
        ...
        if (strAnswer !== 'Cancel' && strAnswer !== 'overlay') {
            ...
            GS.ajaxJSON('/pgmanage/auth', 'action=change_database&database=' + encodeURIComponent(strAnswer), function (data, error) {
                if (!error) {
...
function dialogLocks() {
    ...
    */});
    ...
    GS.openDialog(templateElement, function () {
        GS.requestRawFromSocket(GS.pgmanageProcessDialogSocket, infoQuery.locks, function (data, error) {
...
function dialogOptions(bolOnlyClipboard, currentTableElement) {
    ...
    for (i=0,len=localStorage.length;i < len;i++) {
        ...
        if (/^pgmanageZoom/.test(strProp)) {
            strZoom = strZoom + '<tr>' +
                    '<td>' + strProp.toString().replace(/^pgmanageZoom/,'') + '</td>' +
                    '<td style="width: 10em;"><gs-text id="pgmanage-options-left-panel-' + strProp.toString().replace(/^pgmanageZoom/,'') + '"></gs-text></td>' +
                '</tr>';
        }
    }
...
// open dialog with info about pgmanage
            ...
            */}).replace(/\{\{MINORVERSION\}\}/gim, contextData.minorVersionNumber);
            ...
            GS.openDialog(templateElement, function () {
                ...
                document.getElementById('button-open-export').addEventListener('click', function () {
                    ...
                    window.open('/pgmanage/' + contextData.connectionID + '/export?' + encodeURIComponent(strRequestString));
