...
./envelope/envelope -c ./envelope/config/envelope.conf -d ./envelope/config/envelope-connections-test.conf -t 300 -r ./envelope/web_root -y ./envelope/app -z ./envelope/role -l info -u public_user -w youwontguessthispasswordwillyou1lk2j43k1lj3h4k3j4h56k324j5bnoiusdfyg08 & export ENVELOPEPID="$!"
sleep 5
...
/usr/local/sbin/envelope -d ./envelope/config/envelope-connections-test.conf -u public_user -w youwontguessthispasswordwillyou1lk2j43k1lj3h4k3j4h56k324j5bnoiusdfyg08 & export ENVELOPEPID="$!"
sleep 5
if test $(uname -s) = "OpenBSD"; then
	xdg-open "http://127.0.0.1:8888/test/index.html?http_export=false&ws_file_app=true&ws_file_role=true&ws_file_web_root=true&ws_delete=true&http_file=true&http_upload=true&ws_select=true&ws_insert=true&ws_update=true&_http_auth=true&http_action=true&ws_action=true&ws_info=true" &
	printf "HTTP/1.1 200 OK\r\n\r\n\r\n" | ncat -l -p 45654
