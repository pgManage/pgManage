...
function dependButton(intOid) {
    'use strict';
    return '<gs-button iconleft inline remove-all icon="link" no-focus title="Dependency List" onclick="dependDialog(' + intOid + ', false)">Dependency List</gs-button>' +
            '<gs-button iconleft inline remove-all icon="cubes" no-focus title="Dependency Graph" onclick="dependDialog(' + intOid + ', true)">Dependency Graph</gs-button>';
}

function dependDialog(intOid, bolGraph) {
    'use strict';
	if (bolGraph) {
	    var strHTML = '<iframe style="width: 100%; height: 100%;" src="/postage/' + contextData.connectionID + '/dep_viewer.html?oid=' + intOid + '" />'
...
function statButton(strStatName, intOid, strNamePartOne, strNamePartTwo) {
    'use strict';
    return '<gs-button iconleft inline remove-all icon="bar-chart-o" no-focus title="Object statistics" ' +
                'onclick="' +
...
function dataObjectButtons(strType, intOID, strSchema, strName) {
    ...
    if (strType === 'table' || strType === 'objectTable') {
        strHTML += '<gs-button inline remove-all icon="wrench" title="Design table" ' +
                        'onclick="newTab(\'design-table\', \'' + encodeHTML(strSchema) + '.' + encodeHTML(strName) + '\', '+
                                '{\'oid\': ' + intOID + '});"> Design table</gs-button>';
    ...
    strHTML += '<gs-button inline remove-all icon="table" title="Edit the data in this object" ' +
                    'onclick="newTab(\'datasheet\', \'' + encodeHTML(strSchema) + '.' + encodeHTML(strName) + '\', ' +
                        '{\'queryString\': \'type=' + strType + '&schema=' + encodeHTML(strSchema) + '&object=' + encodeHTML(strName) + '\'})">Edit data</gs-button>';
...
function dumpButton(strOid, strName) {
    return '<gs-button inline remove-all icon="edit" no-focus title="Dump schema to script"'
                    + ' onclick="dialogSchemaSurgery(\'' + strOid + '\', \'' + strName + '\')">Dump schema to script</gs-button>';
}
...
function dialogSchemaSurgery(intSchemaOid, strSchemaName) {
    ...
    */});
	...
	var closeCallback = function (event) {
        ...
        if (strAnswer === 'Open Script') {
            ...
            if (bolOperatorClass) {
                arrQuery.push('\n\n SELECT oid, name, schema_name, \'OperatorClass\' AS objType FROM (' +
                    listQuery.objectOperatorClassDump.replace(/\{\{INTOID\}\}/gim, intSchemaOid).replace(';', '') +
                ') em ');
            }
            if (bolOperatorFamily) {
                arrQuery.push('\n\n SELECT oid, name, schema_name, \'OperatorFamily\' AS objType FROM (' +
                    listQuery.objectOperatorFamilyDump.replace(/\{\{INTOID\}\}/gim, intSchemaOid).replace(';', '') +
                ') em ');
            }
            if (bolOperator) {
            ...
            handleListResults = function (arrResult) {
                ...
                if (bolDropStatments) {
                    ...
                    for (i = arrResult.length - 1; i >= 0; i -= 1) {
                        ...
                        //console.log(arrResult[i][3].toUpperCase());
                        ...
                        } else if (arrResult[i][3].toUpperCase() === 'OWNEDSEQUENCE') {
                            strDumpQuery += '-- DROP SEQUENCE ' + quote_ident(strSchemaName) + '.' + arrResult[i][1] + ';\n';
                        ...
                        } else if (arrResult[i][3].toUpperCase() === 'INDEX') {
                            strDropIndexes += '-- DROP ' + arrResult[i][3].toUpperCase() + ' ' + quote_ident(strSchemaName) + '.' + arrResult[i][1] + ';\n';
                        } else if (arrResult[i][3].toUpperCase() === 'TRIGGERFUNCTION') {
                            strDropTGFunctions += 'DROP ' + arrResult[i][3].toUpperCase().replace('TRIGGERFUNCTION', 'FUNCTION') + ' ' + quote_ident(strSchemaName) + '.' + arrResult[i][1] + ';\n';
                        } else {
                            //console.log(arrResult[i][3].toUpperCase());
                            strDumpQuery += 'DROP ' + arrResult[i][3].toUpperCase() + ' ' + quote_ident(strSchemaName) + '.' + arrResult[i][1] + ';\n';
                        }
                        if (i === 0) {
                            strDumpQuery += strDropIndexes;
                            strDumpQuery += strDropTGFunctions;
                        }
                    }
		...
		GS.triggerEvent(document.getElementById('button-home-back'), 'click', {});
    };
    ...
    setObjectDetailValue(strHTML, function () {
		...
		var arrButton = document.getElementsByClassName('dump-dialog-close');
		for (var i = 0, len = 2; i < len; i += 1) {
			arrButton[i].addEventListener('click', closeCallback);
		}
    });
}
