...
function SQLBeautify(strInput) {
    ...
    */
        ...
        // FOUND a main keyword, no newline INSIDE A GRANT STATEMENT
        ...
        } else if (int_qs === 0 && strInput.substr(i).match(/^\b(((LEFT|FULL[\ \t]+OUTER|FULL|CROSS|LEFT[\ \t]+OUTER|RIGHT|RIGHT[\ \t]+OUTER|INNER)?[\ \t]+)JOIN|RETURNS|SELECT|FROM|GROUP|ORDER|HAVING|WHERE|LIMIT|OFFSET|USING|SET|INSERT|VALUES|FROM[\ \t\n\r]+STDIN)\b/i) && strInput.substr(i - 1, 1).match('^[\n\r\ \t]+')) {
            // Remove previous tab if previous character is whitespace
            if (strResult.substring(strResult.length - 1, strResult.length).match('[\ \t]')) {
                strResult = strResult.substr(0, strResult.length - 1);
            }
            ...
            strResult += '\n' + '\t'.repeat(((intTabLevel < 0) ? 0 : intTabLevel)) + strInput.substr(i).match(/^\b(((LEFT|FULL[\ \t]+OUTER|FULL|CROSS|LEFT[\ \t]+OUTER|RIGHT|RIGHT[\ \t]+OUTER|INNER)?[\ \t]+)JOIN|RETURNS|SELECT|FROM|GROUP|ORDER|HAVING|WHERE|LIMIT|OFFSET|USING|SET|INSERT|VALUES|FROM[\ \t\n\r]+STDIN)\b/i)[0].toUpperCase().trim() + ' ';
            i += (strInput.substr(i).match(/^\b(((LEFT|FULL[\ \t]+OUTER|FULL|CROSS|LEFT[\ \t]+OUTER|RIGHT|RIGHT[\ \t]+OUTER|INNER)?[\ \t]+)JOIN|RETURNS|SELECT|FROM|GROUP|ORDER|HAVING|WHERE|LIMIT|OFFSET|USING|SET|INSERT|VALUES|FROM[\ \t\n\r]+STDIN)\b/i)[0].length - 1);
            bolNoExtraWhitespace = true;
            //console.log(">KEYWORD|" + intTabLevel + "<");
