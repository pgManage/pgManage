                        ...
                        try {
                            ...
                            if (editor.currentSelections.length > 1) {
                                if (
                                    ...
                                    selectionRanges.start.row === event.start.row &&
                                    selectionRanges.start.column === event.start.column + 1 &&
                                    selectionRanges.end.row === event.end.row &&
        ...
        if (autocompleteGlobals.arrCancelledIds.indexOf(intLoadId) === -1) {
            if (bolLast === true) {
                ...
                if (autocompleteTempList.length === 1) {
                    ...
                    strText += '\n' + strCurrent;
                    autocompleteGlobals.arrSearch.push('tk ' + strSearch);
                    autocompleteGlobals.arrValues.push('tk ' + strCurrent);
                    autocompleteGlobals.arrSearchMaster.push('tk ' + strSearch);
                    autocompleteGlobals.arrValuesMaster.push('tk ' + strCurrent);
                    ...
                    for (var i = 0, len = autocompleteTempList.length; i < len; i++) {
                        ...
                        if (bolIsFunction) {
                            ...
                            strText += '\n' + nextItem;
                            autocompleteGlobals.arrSearch.push('FN ' + strSearch);
                            autocompleteGlobals.arrValues.push('FN ' + nextItem);
                            autocompleteGlobals.arrSearchMaster.push('FN ' + strSearch);
                            autocompleteGlobals.arrValuesMaster.push('FN ' + nextItem);
                ...
                if (autocompleteGlobals.bolSnippets) {
                    for (var i = 0, len = snippets.length; i < len; i++) {
                        ...
                        strSearch = strCurrent;
                        strText += '\n' + strCurrent;
                        autocompleteGlobals.arrSearch.push('SN ' + strSearch);
                        autocompleteGlobals.arrValues.push('SN ' + strCurrent);
                        autocompleteGlobals.arrSearchMaster.push('SN ' + strSearch);
                        autocompleteGlobals.arrValuesMaster.push('SN ' + strCurrent);
                ...
                if (autocompleteGlobals.arrVariables && autocompleteGlobals.arrVariables.length > 0) {
                    for (var i = 0, len = autocompleteGlobals.arrVariables.length; i < len; i++) {
                        ...
                        autocompleteGlobals.arrSearch.push('VR ' + strSearch);
                        autocompleteGlobals.arrValues.push('VR ' + strCurrentVar);
                        autocompleteGlobals.arrSearchMaster.push('VR ' + strSearch);
                        autocompleteGlobals.arrValuesMaster.push('VR ' + strCurrentVar);
                ...
                function CompareStrsValue (a, b) {
                    if (a.substring(3).toLowerCase() < b.substring(3).toLowerCase()) return -1;
                    if (a.substring(3).toLowerCase() > b.substring(3).toLowerCase()) return 1;
                    return 0;
                }
                // function CompareStrsSearch (a, b) {
                ...
                autocompleteGlobals.arrValuesMaster = distinctValues.sort(CompareStrsValue);
                autocompleteGlobals.arrSearchMaster = distinctSearch.sort(CompareStrsValue);
                autocompleteGlobals.arrValues = distinctValues.sort(CompareStrsValue);
                autocompleteGlobals.arrSearch = distinctSearch.sort(CompareStrsValue);
                ...
                if (editor.currentSelections.length > 1) {
                    ...
                    for (var i = 0, len = editor.currentSelections.length; i < len; i += 1) {
                        ...
                        if (autocompleteGlobals.searchLength !== 1) {
                            insertText = autocompleteGlobals.arrValues[intFocusedLine].trim().substring(autocompleteGlobals.searchLength - 1 + 3, autocompleteGlobals.arrValues[intFocusedLine].trim().length);
                        } else {
                            insertText = autocompleteGlobals.arrValues[intFocusedLine].trim().substring(autocompleteGlobals.searchLength + 3, autocompleteGlobals.arrValues[intFocusedLine].trim().length);
                        }
                        insertObj = {
